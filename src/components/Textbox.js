...
const Textbox = () => {
    ...
    const [BSTimer, setBSTimer] = useState(setTimeout(() => null, 0));
    const highlightYellow = (str) => `<span class='bg-amber-300 rounded'>${str}</span>`;
    useLayoutEffect(() => {
    ...
    return (_jsx("div", { className: "w-full h-full flex-auto flex justify-center items-top overflow-x-auto fancyscroll", children: _jsx("div", { suppressContentEditableWarning: true, contentEditable: "true", ref: textDiv, spellCheck: "false", className: "resize-none outline-none p-2  xs:py-4 xs:px-16 w-full md:w-3/4 h-full whitespace-pre-wrap overflow-x-hidden max-w-[1280px] font-textfont text-sneucolor-150 overflow-y-auto break-words", onKeyUp: handleKeyUp, onKeyDown: handleKeyDown, onPaste: () => doUndo([state, dispatch]), onCut: () => doUndo([state, dispatch]) }, void 0) }, void 0));
};
